...
class GameProvider with ChangeNotifier {
  ...
  // Error handling and crash prevention system
  ...
  GameProvider({
    ...
    _performanceMonitor.startMonitoring(
      onPerformanceWarning: _handlePerformanceWarning,
  ...
  Future<void> _initializeErrorRecovery() async {
    try {
      ...
      // ゲーム状態の復元を試行
      final savedState = await _errorRecoveryManager.restoreGameState();
      if (savedState != null) {
        await _restoreFromSavedState(savedState);
      }
    } catch (error, stackTrace) {
  ...
  Future<void> _restoreFromSavedState(GameStateSnapshot savedState) async {
    try {
      ...
      if (savedState.gameState == 'running' && 
          DateTime.now().difference(savedState.timestamp).inMinutes < 30) {
        ...
        _pushToast(
          const GameToast(
            message: 'Previous game restored',
  ...
  // ゲーム状態の定期保存
  Future<void> _saveCurrentGameState() async {
    if (_gameState == GameState.running) {
      ...
      await _errorRecoveryManager.saveGameState(snapshot);
    }
  }

  // ネットワークエラーハンドリング
  Future<void> _handleNetworkError(Object error) async {
    final shouldContinueOffline = await _errorRecoveryManager.handleNetworkError(error);
    
    if (shouldContinueOffline) {
      _pushToast(
        const GameToast(
          message: 'Playing offline - progress saved locally',
  ...
  void _performEmergencyCleanup() {
    try {
      ...
      // 古い障害物を削除
      obstacleProvider.performEmergencyCleanup();
      
      // コインエフェクトをクリア
      coinProvider.clearEffects();
      
    } catch (error, stackTrace) {
      _errorRecoveryManager.handleCrash(error, stackTrace);
    }
  }
  ...
  /// パフォーマンス警告の処理
  void _handlePerformanceWarning() {
    debugPrint('GameProvider: パフォーマンス警告 - フレームレート低下検知');
    
    // 自動最適化の適用
    if (_batteryOptimizer.shouldLimitGpuUsage()) {
      // GPU使用率制限時は描画品質を下げる
      _pushToast(
        const GameToast(
  ...
  void _handleMemoryWarning() {
    debugPrint('GameProvider: メモリ警告 - 使用量が閾値を超過');
    
    // メモリ最適化の実行
    lineProvider.clearOldLines();
    obstacleProvider.clearDistantObstacles();
    coinProvider.clearDistantCoins();
  ...
  void _handleLowBattery() {
    debugPrint('GameProvider: 低バッテリー警告 - 省電力モード有効');
    
    // 省電力モードの適用
    _batteryOptimizer.applyPowerSavingProfile(PowerSavingLevel.moderate);
  ...
  void _handlePowerModeChanged() {
    if (_batteryOptimizer.isCharging) {
      debugPrint('GameProvider: 充電開始 - パフォーマンスモード復帰');
      _batteryOptimizer.applyPowerSavingProfile(PowerSavingLevel.none);
    } else {
      debugPrint('GameProvider: 充電停止 - バッテリー最適化モード');
      _batteryOptimizer.applyPowerSavingProfile(PowerSavingLevel.moderate);
    }
  }
  ...
  Map<String, dynamic> getPerformanceStatistics() {
    return {
      ...
      'battery': _batteryOptimizer.getBatteryStatistics(),
      'gameState': {
        'currentFps': _performanceMonitor.averageFps,
        'targetFps': _batteryOptimizer.targetFrameRate,
        'droppedFrames': _performanceMonitor.droppedFrames,
  ...
  void updateOptimizationSettings({
    ...
    PowerSavingLevel? powerSavingLevel,
  }) {
    if (enablePerformanceMonitoring == false) {
      _performanceMonitor.stopMonitoring();
    } else if (enablePerformanceMonitoring == true) {
      _performanceMonitor.startMonitoring(
        onPerformanceWarning: _handlePerformanceWarning,
        onMemoryWarning: _handleMemoryWarning,
      );
    ...
    if (enableBatteryOptimization == false) {
      _batteryOptimizer.stopOptimization();
    } else if (enableBatteryOptimization == true) {
      _batteryOptimizer.startOptimization(
        onLowBattery: _handleLowBattery,
        onPowerModeChanged: _handlePowerModeChanged,
      );
    ...
    if (powerSavingLevel != null) {
      _batteryOptimizer.applyPowerSavingProfile(powerSavingLevel);
    }
  }
